...
def export_violation_official_pdf(request, pk):
    ...
    if getattr(violation, 'violation_number', None):
        write_box(page1, fitz.Rect(210, 186, 385, 204), str(violation.violation_number) if str(violation.violation_number).startswith("(") else f"({violation.violation_number})", fontsize=10)
